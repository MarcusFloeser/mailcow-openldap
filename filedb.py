...
class DbAlias(Base): # type: ignore
...
def get_unchecked_active_aliases():
    query = session.query(DbAlias.email).filter(DbAlias.last_seen != session_time).filter(DbAlias.active == True)

    return [x.email for x in query]

def add_alias(email, active=True):
    session.add(DbAlias(email=email, active=active, last_seen=session_time))
    session.commit()

def check_alias(email):
    user = session.query(DbAlias).filter_by(email=email).first()
    if user is None:
        return (False, False)
    user.last_seen = session_time
    session.commit()
...
def alias_set_active_to(email, active):
    user = session.query(DbAlias).filter_by(email=email).first()
    user.active = active
    session.commit()