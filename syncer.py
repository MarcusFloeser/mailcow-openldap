...
def main():    
    ...
    while (True):
        logging.info(f"Start sync for mail aliases ... ")
        sync_alias()
        logging.info(f"Start sync for mailboxes ... ")
        sync_mailbox()
...
def sync_alias():
    ...
    ldap_results = ldap_connector.search_s(config['LDAP_ALIAS_BASE_DN'], ldap.SCOPE_SUBTREE, 
                config['LDAP_ALIAS_FILTER'], 
                ['mail', 'uid'])
    ...
    for entry in ldap_results:
        ldap_active = True
        mail = ""
        uid = entry[1]['uid'][0].decode()
        domain = uid.split('@')[1]
        # sort mail list
        entry[1]['mail'].sort()
        for m in entry[1]['mail']:
            dec = m.decode()
            mail = mail + dec + ","

        # normailze mail address list (remove last comma)
        mail = mail[:-1]

        (db_user_exists, db_user_active) = filedb.check_alias(uid)
        (api_user_exists, api_user_active, api_goto) = api.check_alias(uid)

        unchanged = True

        if not db_user_exists:
            filedb.add_alias(uid, ldap_active)
            (db_user_exists, db_user_active) = (True, ldap_active)
            logging.info (f"Added filedb user: {uid} (Active: {ldap_active})")
            unchanged = False

        if not api_user_exists:
            api.add_alias(domain, uid, mail ,ldap_active)
            (api_user_exists, api_user_active, api_goto) = (True, ldap_active, mail)
            logging.info (f"Added Mailcow Alias: {uid} (Active: {ldap_active})")
            unchanged = False

        if db_user_active != ldap_active:
            filedb.alias_set_active_to(uid, ldap_active)
            logging.info (f"{'Activated' if ldap_active else 'Deactived'} {uid} in filedb")
            unchanged = False

        if api_user_active != ldap_active:
            api.edit_alias(uid, active=ldap_active)
            logging.info (f"{'Activated' if ldap_active else 'Deactived'} {uid} in Mailcow")
            unchanged = False

        if api_goto != mail:
            api.edit_alias(uid, goto=mail)
            logging.info (f"Changed Alias {uid} in Mailcow: {mail}")
            unchanged = False

        if unchanged:
            logging.info (f"Checked alias {uid}, unchanged")

    for uid in filedb.get_unchecked_active_aliases():
        (api_user_exists, api_user_active, _) = api.check_alias(uid)

        if (api_user_active and api_user_active):
            api.delete_alias(uid)
            logging.info (f"Deleted alias {uid} in Mailcow, not found in LDAP")
        
        filedb.alias_set_active_to(uid, False)
        logging.info (f"Deactivated alias {uid} in filedb, not found in LDAP")
...
def read_config():
    required_config_keys = [
        ...
        'LDAP-MAILCOW_LDAP_ALIAS_BASE_DN',
    ...
    config['LDAP_ALIAS_FILTER'] = os.environ['LDAP-MAILCOW_LDAP_ALIAS_FILTER'] if 'LDAP-MAILCOW_LDAP_ALIAS_FILTER' in os.environ else "objectClass='posixAccount'"
