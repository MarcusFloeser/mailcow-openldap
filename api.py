...
def add_alias(domain, email, goto, active):
...
def edit_alias(email, active=None, goto=None):
    attr = {}
...
def check_alias(email):
    url = f"{api_host}/api/v1/get/alias/{email}"
    headers = {'X-API-Key': api_key, 'Content-type': 'application/json'}
    ...
    return (True, bool(rsp['active_int']), rsp['goto'])
